...
void fixReadInvalidCharacters(char *buffer, int readValue, FILE *stream) {
	int i;
//	fwrite(timeBuffer, sizeof(char), 9, stream);
	for (i = 0; i < readValue; i++) {
		if (buffer[i] == 9 || buffer[i] == 10 || buffer[i] == 13 ||
...
//					timeBuffer = timestamp();
					fixReadInvalidCharacters(buffer, readValue, outputFile);
					fixReadInvalidCharacters(buffer, readValue, stdout);
